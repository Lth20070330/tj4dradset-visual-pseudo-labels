...
class RadarDetectionDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        if self.metadata_dir is not None:
            import json
            records = json.loads((self.metadata_dir / f"{frame_id}.json").read_text(encoding="utf-8"))
        ...
        weight = np.zeros((1, self.config.height, self.config.width), dtype=np.float32)
        for object_index, obj in enumerate(objects):
            ...
            weight[0, row, col] = qualities[object_index] if qualities is not None and object_index < len(qualities) else 1.0
        return {
            "frame_id": frame_id,
...
def detection_loss(outputs: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, dict[str, float]]:
    ...
    number_of_positives = positives.sum().clamp_min(1.0)
    ...
    regression_loss = (regression_error * mask).sum() / (mask.sum().clamp_min(1.0) * outputs["regression"].shape[1])
